...
const RecipeCards = () => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`http://localhost:5000/api/recipes/${category}`);
        setRecipes(res.data);
      } catch (err) {
        console.error(err);
        setRecipes([]);
  ...
  const handleAskAI = () => {
    navigate("/ask-ai"); // 👈 change this path to your actual AI page route if needed
  };
      ...
      {loading ? (
        ...
        <div className="text-center mt-20">
          <p className="text-gray-600 text-lg mb-4">
            Hmm... No recipes found for this category.
          </p>
          <button
            onClick={handleAskAI}
            className="inline-flex items-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-white font-medium px-6 py-3 rounded-full shadow-md transition-all duration-300"
          >
